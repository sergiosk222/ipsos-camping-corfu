...
function upgradeHero() {
  ...
  hero?.classList.add('cinematic-boost');
  ...
  if (heroArt && !heroArt.querySelector('.floating-card')) {
    heroArt.insertAdjacentHTML('beforeend', `
      <div class="floating-card float-one">Available places</div>
...
function insertPerfectSection() {
  ...
  facilities.insertAdjacentHTML('beforebegin', `
    <section class="v2-section reveal perfect-section">
      ...
      <div class="v2-grid">
        <article class="v2-card"><b>01</b><h3>Arrive easily</h3><p>Simple access for camper vans and travellers who need a convenient stop in Ipsos.</p></article>
        <article class="v2-card"><b>02</b><h3>Stay close to everything</h3><p>Beach, food, shops and local movement are nearby, so you do not waste time.</p></article>
        <article class="v2-card"><b>03</b><h3>Confirm before arrival</h3><p>Send your request by WhatsApp and wait for owner confirmation before you arrive.</p></article>
      </div>
    </section>
  `);
}
      ...
      <div class="booking-v2-layout">
        <aside class="booking-box" id="dates">
          ...
          <div class="booking-summary-mini">
            <span id="nightsCount">Select dates</span>
            <span id="selectedPlaceLabel">No place selected</span>
          </div>

          <div class="selected-place-v2" id="selectedCard">
            <p class="eyebrow">Selected place</p>
            <h3 id="selectedSpotName">Tap an available place</h3>
            <p id="selectedSpotText">Choose a green spot on the campsite map to see details here.</p>
            <div class="spot-features-v2" id="selectedSpotFeatures">
        ...
        <div class="campsite-map-visual" id="map">
          <div class="map-sea-label">🌊 Ipsos Beach</div>
          <div class="map-road-label">Access road</div>
          <div class="map-zone-label">Tent garden</div>
          <div class="visual-road"></div>
          <div class="visual-path"></div>
          <div class="map-building-v2 da-marco-v2"><strong>Da Marco</strong><small>Restaurant</small></div>
          <div class="map-building-v2 reception-v2"><strong>Reception</strong><small>Call first</small></div>
          <div class="map-building-v2 showers-v2"><strong>WC</strong><small>Showers</small></div>
          <div class="map-tree tree-a"></div>
          <div class="map-tree tree-b"></div>
          <div class="map-tree tree-c"></div>
        ...
        <form class="booking-box" id="bookingForm">
          <p class="eyebrow" id="details">Step 3</p>
          <h3>Guest details</h3>
          <input type="hidden" id="selectedPlace" />
          <input type="hidden" id="selectedPlaceType" />
          <label>Name<input type="text" id="guestName" placeholder="Your name" required /></label>
          <label>Phone / WhatsApp<input type="tel" id="guestPhone" placeholder="+30 ..." required /></label>
          <label>Email<input type="email" id="guestEmail" placeholder="your@email.com" /></label>
          <label>Vehicle / stay type<select id="stayType"><option>Camper van</option><option>Caravan</option><option>Tent</option><option>Not sure yet</option></select></label>
          <label>Message<textarea id="guestMessage" rows="4" placeholder="Vehicle size, pets, electricity, late arrival or special requests."></textarea></label>
          <button class="btn primary" type="submit" id="send">Prepare request</button>
          <p class="booking-note">This opens WhatsApp with a prepared message. Real database availability can be added later.</p>
        </form>
      </div>

      <div class="booking-result" id="bookingResult" hidden>
        <div>
          <p class="eyebrow">Request ready</p>
          <h3>Your booking request is prepared</h3>
          <p id="bookingResultText"></p>
        </div>
        <div class="booking-result-actions">
          <a class="btn primary" id="bookingWhatsApp" href="#" target="_blank" rel="noreferrer">Send on WhatsApp</a>
          <button class="btn ghost" id="copyBooking" type="button">Copy request</button>
        </div>
      </div>
    </div>
  `;
...
function insertHowBookingWorks() {
  ...
  booking.insertAdjacentHTML('afterend', `
    <section class="v2-section reveal how-booking-v2">
      <p class="eyebrow">How booking works</p>
      <h2>Simple request, manual confirmation</h2>
      <div class="how-grid-v2">
        <article class="v2-card"><b>1</b><h3>Choose dates</h3><p>Select arrival and departure dates.</p></article>
        <article class="v2-card"><b>2</b><h3>Pick a place</h3><p>Tap an available place on the campsite map.</p></article>
        <article class="v2-card"><b>3</b><h3>Send request</h3><p>Send the prepared WhatsApp message.</p></article>
        <article class="v2-card"><b>4</b><h3>Wait confirmation</h3><p>Your place is confirmed only after owner reply.</p></article>
      </div>
    </section>
  `);
}
...
function upgradeGallery() {
  ...
  if (!gallery || gallery.querySelector('.journal-gallery')) return;

  gallery.innerHTML = `
    <p class="eyebrow">Travel journal</p>
    <h2>Real photos can be added later</h2>
    <p class="section-text">The gallery now works like a travel board. Replace these placeholders with real camping, restaurant and beach photos when available.</p>
    <div class="journal-gallery">
      <article><div>🌿</div><h3>Green camping area</h3><p>Under trees, close to Ipsos.</p></article>
      <article><div>🚐</div><h3>Camper spaces</h3><p>Easy arrival and simple stay.</p></article>
      <article><div>🍕</div><h3>Da Marco evenings</h3><p>Food and warm summer light.</p></article>
      <article><div>🌊</div><h3>Ipsos Beach</h3><p>Sea air and beach access.</p></article>
      <article><div>🫒</div><h3>Corfu nature</h3><p>Olive trees and greenery.</p></article>
    </div>
  `;
}
...
function addStickyBookingCta() {
  ...
  document.body.insertAdjacentHTML('beforeend', `
    <div class="sticky-booking-cta">
      <a href="#booking">Plan your stay</a>
      <a href="https://wa.me/306982875538" target="_blank" rel="noreferrer">WhatsApp</a>
    </div>
  `);
}
...
function setupBookingV2() {
  ...
  document.querySelectorAll('.map-spot-v2.available').forEach(button => {
    button.addEventListener('click', () => selectSpot(button));
  });

  document.querySelectorAll('.wizard-steps-v2 button').forEach(button => {
    button.addEventListener('click', () => {
      document.querySelectorAll('.wizard-steps-v2 button').forEach(item => item.classList.remove('active'));
      button.classList.add('active');
      document.getElementById(button.dataset.focus)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
  });
  ...
  copyBooking?.addEventListener('click', async () => {
    ...
    try {
      ...
      setTimeout(() => { copyBooking.textContent = 'Copy request'; }, 1800);
    } catch (error) {
